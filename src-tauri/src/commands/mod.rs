...
pub async fn start_processing(
    ...
    for job in queued_jobs {
        ...
        for format in &config.render.export_formats {
            ...
            let result = if result.is_ok() && needs_hook {
                ...
                if let Err(e) = intro_result {
                    ...
                    // Check if main output has audio (silent source videos won't)
                    ...
                    // Concatenate intro + main → final output using concat filter
        ...
        // Clean up temp files from typewriter hook
        if temp_dir.exists() {
